      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          {editingInfo ? (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input
                  ...
                  value={editForm.mbti || ""}
                  onChange={(e) =>
                    setEditForm((f) => ({ ...f, mbti: e.target.value }))
                  }
                  className="rounded border border-gray-300 px-3 py-2 text-sm"
                />
                <input
                  ...
                  onChange={(e) =>
                    setEditForm((f) => ({ ...f, birth_year: e.target.value }))
                  }
                  maxLength={2}
                  className="rounded border border-gray-300 px-3 py-2 text-sm"
                />
              ...
              <input
                placeholder="만남 경위"
                value={editForm.meeting_reason || ""}
                onChange={(e) =>
                  setEditForm((f) => ({
                    ...f,
                    meeting_reason: e.target.value,
                  }))
                }
            ...
            <div className="space-y-2 text-sm">
              {life.student_id_number && (
                <InfoRow label="학번" value={life.student_id_number} />
              )}
              {life.age && <InfoRow label="나이" value={`${life.age}세`} />}
              {life.grade && <InfoRow label="학년" value={life.grade} />}
              {life.department && (
                <InfoRow label="학과" value={life.department} />
              )}
              {life.mbti && <InfoRow label="MBTI" value={life.mbti} />}
              {life.meeting_reason && (
                <InfoRow label="만남 경위" value={life.meeting_reason} />
              )}
              {life.has_partner !== null && (
                <InfoRow
                  label="이성 유무"
                  value={life.has_partner ? "있음" : "없음"}
                />
              )}
              {life.characteristics && (
                <InfoRow label="특징" value={life.characteristics} />
              )}
              {!life.student_id_number &&
                !life.age &&
                !life.department &&
                !life.mbti && (
                  <p className="text-gray-400 text-xs">
                    기본 정보가 아직 없습니다. 수정 버튼을 눌러 입력해주세요.
                  </p>
                )}
            </div>
          )}
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={() =>
                router.push(`/student/life/${lifeId}/journal/new`)
              }
              className="text-xs text-blue-500"
            >
              + 일지 추가
            ...
            <div className="space-y-3">
              {journals.map((j) => (
                <button
                  ...
                  onClick={() =>
                    router.push(
                      `/student/life/${lifeId}/journal/${j.id}`
                    )
                  }
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-500">
                      {new Date(j.met_date).toLocaleDateString("ko-KR")}
                    </span>
                    <span className="text-xs text-gray-400">
                      {(j.author as any)?.display_name || ""}
                    </span>
                  </div>
