...
export default function CheckinPage() {
    ...
    if (attendee.life_id) {
      ...
      if (!existing || existing.length === 0) {
        await supabase.from("journals").insert({
          life_id: attendee.life_id,
          met_date: today,
          location: eventName,
          response: `[${eventName}] 출석완료`,
          purpose: "management",
        });
      }
    }
  ...
  const fieldLabels: Record<string, string> = {
    department: "학과", team: "팀", year: "학번", gender: "성별", phone: "연락처", friend_group: "친구",
  };
            ...
            <div className="text-center space-y-4 w-full max-w-sm">
              <div className="bg-green-50 border border-green-200 rounded-2xl p-6 space-y-3">
                ...
                {showFields.map((f: string) => (
                  result.attendee[f] && (
                    <p key={f} className="text-sm text-gray-600">{fieldLabels[f] || f}: {result.attendee[f]}</p>
                  )
                ))}
