...
export default function NewLifePage() {
  ...
  const handleLinkExisting = async (lifeId: string) => {
    if (!user) return;
    setLoading(true);
    const { error: ulErr } = await supabase.from("user_lives").upsert({ user_id: user.id, life_id: lifeId, role_in_life: "manager" }, { onConflict: "user_id,life_id", ignoreDuplicates: true });
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      <header className="shrink-0 bg-white border-b border-gray-200 px-4 py-3 flex items-center">
        <button onClick={() => { if (step !== "name") setStep("name"); else if (window.history.length > 1) router.back(); else router.push("/manager"); }} className="text-gray-500 mr-3">&larr;</button>
